...
function collectTransactions(value: unknown, out: Array<{ to: string; data: string; value?: string }> = []) {
  if (Array.isArray(value)) { for (const item of value) collectTransactions(item, out); return out; }
  ...
  if (to && data && /^0x[0-9a-fA-F]{40}$/.test(to) && /^0x[0-9a-fA-F]*$/.test(data)) {
    ...
    if (!out.some((tx) => `${tx.to.toLowerCase()}:${tx.data.toLowerCase()}:${tx.value || "0"}` === key)) out.push({ to, data, ...(valueField ? { value: valueField } : {}) });
  }
...
export async function POST(req: Request) {
  ...
  if (!kitKey) return NextResponse.json({ error: "Circle App Kit is not configured on the server." }, { status: 503 });
  ...
  try {
    ...
    provider = { request, on: () => provider, removeListener: () => provider };
    ...
    const estimate = await kit.estimateSwap({
      ...
      tokenOut,
      amountIn: amount,
      config: { kitKey, slippageBps: 100, allowanceStrategy: "approve" },
    });
    const transactions = collectTransactions(estimate);
    if (!transactions.length) return NextResponse.json({ error: "Circle App Kit returned a quote without executable transaction data.", estimate: jsonSafe(estimate) }, { status: 502 });
    return NextResponse.json({ ok: true, chainId: ARC_CHAIN_ID, address, tokenIn, tokenOut, amountIn: amount, estimate: jsonSafe(estimate), transactions: jsonSafe(transactions) }, { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    console.error("[AGFusion] Circle App Kit swap preparation failed", error);
    return NextResponse.json({ error: message || "Circle App Kit could not prepare this swap." }, { status: 502 });
  }
}
