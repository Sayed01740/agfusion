...
export default function DashboardPage() {
  ...
  return (
    <main className="ag-premium-shell mx-auto w-full max-w-[1440px] overflow-x-clip pb-[calc(1.5rem+env(safe-area-inset-bottom))] sm:px-5 sm:py-6 lg:px-8 lg:py-8">
      <header className="mb-4 px-2 pt-2 sm:mb-5 sm:px-0 sm:pt-0">
        <div className="ag-command-hero overflow-hidden px-4 py-5 sm:px-6 sm:py-7 lg:px-8 lg:py-9">
          <div className="relative z-10 flex min-w-0 flex-col gap-5 sm:gap-6 lg:gap-8">
            <div className="flex min-w-0 items-start justify-between gap-3 sm:gap-6">
              <div className="min-w-0 max-w-3xl">
                <div className="mb-2 flex items-center gap-2 text-[9px] font-semibold uppercase tracking-[0.18em] text-cyan-300/75 sm:text-[11px]">
                  <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-cyan-300 shadow-[0_0_12px_rgba(103,232,249,.6)]" />
                  Command center
                </div>
                <h1 className="ag-hero-title max-w-[18ch] text-[clamp(1.85rem,8.5vw,4.6rem)] leading-[0.98] sm:max-w-none">Move money <em>intelligently.</em></h1>
                <p className="mt-2.5 max-w-2xl text-[12px] leading-[1.55] text-slate-400 sm:mt-3 sm:text-[15px] sm:leading-6">One workspace for AI-assisted stablecoin operations across Arc and supported EVM networks.</p>
              </div>
            ...
            <div className="grid grid-cols-2 gap-px overflow-hidden rounded-2xl border border-white/[0.07] bg-white/[0.06] sm:grid-cols-3">
              <div className="min-w-0 bg-black/10 px-3.5 py-3.5 sm:px-5 sm:py-5">
                <p className="ag-metric-label truncate text-[9px] sm:text-[10px]">Available balance</p>
                <p className="ag-metric-value mt-1 truncate text-base sm:text-xl">{liveBalanceUsdc ? `${formatUsdc(Number(liveBalanceUsdc))} USDC` : "—"}</p>
              </div>
              <div className="min-w-0 bg-black/10 px-3.5 py-3.5 sm:px-5 sm:py-5">
                <p className="ag-metric-label truncate text-[9px] sm:text-[10px]">Connected wallet</p>
                <p className="ag-metric-value mt-1 truncate text-[12px] sm:text-[14px]">{walletAddress ? shortenAddress(walletAddress) : "Not connected"}</p>
              </div>
              <div className="col-span-2 min-w-0 bg-black/10 px-3.5 py-3.5 sm:col-span-1 sm:px-5 sm:py-5">
                <p className="ag-metric-label truncate text-[9px] sm:text-[10px]">Operations</p>
                <p className="ag-metric-value mt-1 text-base sm:text-xl">{transactions.length.toString().padStart(2, "0")} <span className="text-[10px] font-medium text-slate-500 sm:text-xs">recorded</span></p>
              </div>
            </div>
        ...
        {payRequest && (
          <div className="mt-3 flex flex-col gap-2 rounded-2xl border border-cyan-500/20 bg-cyan-500/[0.045] p-3 text-xs text-cyan-50 sm:flex-row sm:items-center sm:justify-between sm:px-4">
            ...
            <Button size="sm" className="h-10 w-full shrink-0 sm:w-auto" disabled={!walletAddress || payBusy} onClick={() => void fulfillPayRequest()}>{payBusy ? "Signing…" : "Pay request"}</Button>
          </div>
        )}
      </header>

      <div className="px-2 sm:px-0">
        <div className="mb-3 grid grid-cols-2 rounded-2xl border border-white/[0.07] bg-white/[0.018] p-1 lg:hidden">
          <button onClick={() => setMobileTab("chat")} className={cn("flex min-h-12 items-center justify-center gap-2 rounded-xl text-[13px] font-semibold transition active:scale-[0.98]", mobileTab === "chat" ? "bg-cyan-400/[0.08] text-cyan-50 ring-1 ring-cyan-400/20" : "text-slate-500 hover:text-slate-200")}>
            <MessageSquare className={cn("h-4 w-4", mobileTab === "chat" ? "text-cyan-300" : "text-slate-500")} /> AI Operator
          </button>
          <button onClick={() => setMobileTab("tools")} className={cn("flex min-h-12 items-center justify-center gap-2 rounded-xl text-[13px] font-semibold transition active:scale-[0.98]", mobileTab === "tools" ? "bg-white/[0.06] text-slate-50 ring-1 ring-white/10" : "text-slate-500 hover:text-slate-200")}>
            <Wrench className={cn("h-4 w-4", mobileTab === "tools" ? "text-slate-200" : "text-slate-500")} /> Operate
          </button>
        </div>
      </div>

      <section className="grid w-full min-w-0 gap-4 px-2 pb-2 sm:gap-5 sm:px-0 lg:grid-cols-12">
        <div className={cn("min-w-0 w-full lg:col-span-7 xl:col-span-8", mobileTab !== "chat" && "hidden lg:block")}>
          <div className="[&>div]:!h-[clamp(430px,calc(100dvh-330px),680px)] [&>div]:!min-h-[430px] sm:[&>div]:!h-full sm:[&>div]:!min-h-[520px]">
            <ChatPanel />
          </div>
        </div>
          ...
          <Card className="w-full overflow-hidden">
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between gap-3">
                <CardTitle className="truncate text-sm">Activity timeline</CardTitle>
                <a href="#tools" className="inline-flex min-h-9 shrink-0 items-center gap-1 rounded-lg px-2 text-[11px] text-cyan-400/90 hover:text-cyan-300">Operate <ArrowRight className="h-3 w-3" /></a>
              </div>
            </CardHeader>
            <CardContent className="space-y-2">
              ...
              {transactions.slice(0, 8).map((tx) => {
                ...
                return (
                  <button key={tx.id} type="button" onClick={() => setActiveTx(tx.id)} className={cn("flex min-w-0 w-full min-h-12 items-center justify-between gap-2 rounded-xl border px-3 py-3 text-left text-sm transition active:scale-[0.995]", isActive ? "border-cyan-500/30 bg-cyan-500/[0.06]" : "border-white/[0.05] bg-white/[0.018] hover:border-white/10")}>
