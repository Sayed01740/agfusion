...
function buildPreview(intent: ParsedIntent): ActionPreview | undefined {
  ...
  switch (intent.type) {
    case "bridge": {
      ...
      if (!complete) return { type: "bridge", title: "Bridge USDC", summary: "I need the amount and both source and destination networks before I can prepare a bridge.", amount: intent.amount, token: intent.token, fromChain: intent.fromChain, toChain: intent.toChain, plan: [{ id: "details", label: "Specify amount + source + destination" }], canExecute: false, requiresWallet: true };
      const est = estimateBridgeDemo(intent.amount!, intent.fromChain!, intent.toChain!);
      return { type: "bridge", title: "Transfer USDC", summary: `Move ${intent.amount} ${intent.token} from ${from?.label ?? "source"} → ${to?.label ?? "destination"}`, amount: intent.amount, token: intent.token, fromChain: intent.fromChain, toChain: intent.toChain, estimatedFeeUsd: est.feeUsd + est.gasUsd, estimatedTime: est.eta, route: [from?.short ?? "?", "→", to?.short ?? "?"], plan: [{ id: "est", label: "Estimate fees", detail: formatUsd(est.feeUsd + est.gasUsd) }, { id: "approve", label: "Approve USDC" }, { id: "burn", label: "Lock on source" }, { id: "attest", label: "Attestation" }, { id: "mint", label: "Credit on destination" }], canExecute: true, requiresWallet: true };
    }
    case "swap": {
      const complete = hasAmount && Boolean(intent.token && intent.tokenOut && intent.toChain);
      if (!complete) return { type: "swap", title: "Swap tokens", summary: "I need the amount, input/output tokens, and network before I can prepare a swap.", amount: intent.amount, token: intent.token, tokenOut: intent.tokenOut, toChain: intent.toChain, plan: [{ id: "details", label: "Specify amount + token pair + network" }], canExecute: false, requiresWallet: true };
      const est = estimateSwapDemo(intent.amount!, intent.token!, intent.tokenOut!);
      return { type: "swap", title: `Swap ${intent.token} → ${intent.tokenOut}`, summary: `Exchange ${intent.amount} ${intent.token} for ~${est.amountOut} ${intent.tokenOut} on ${to?.label ?? "the selected network"}`, amount: intent.amount, token: intent.token, tokenOut: intent.tokenOut, toChain: intent.toChain, estimatedFeeUsd: est.feeUsd, estimatedTime: "~2s", route: [intent.token!, "FX", intent.tokenOut!], plan: [{ id: "quote", label: "Get FX quote", detail: `~${est.amountOut} ${intent.tokenOut}` }, { id: "approve", label: "Approve / permit" }, { id: "swap", label: "Execute swap" }], canExecute: true, requiresWallet: true };
    }
    case "send": {
      const complete = hasAmount && Boolean(intent.recipient && intent.toChain);
      return { type: "send", title: intent.recipientLabel ? `Pay ${intent.recipientLabel}` : "Send payment", summary: complete ? `Send ${intent.amount} ${intent.token} to ${intent.recipient} on ${to?.label ?? "the selected network"}` : "I need a valid 0x recipient, amount, and network before I can prepare the payment.", amount: intent.amount, token: intent.token, toChain: intent.toChain, recipient: intent.recipient, recipientLabel: intent.recipientLabel, estimatedFeeUsd: complete ? 0.02 : undefined, estimatedTime: complete ? "<1s" : undefined, plan: complete ? [{ id: "resolve", label: "Resolve recipient" }, { id: "estimate", label: "Estimate gas" }, { id: "sign", label: "Sign & send" }] : [{ id: "details", label: "Specify amount + valid recipient + network" }], canExecute: complete, requiresWallet: true };
    }
    case "route": {
      const complete = hasAmount && Boolean(intent.fromChain && intent.toChain && intent.recipient && intent.fromChain !== intent.toChain);
      return { type: "route", title: "Transfer + pay", summary: complete ? `Move ${intent.amount} ${intent.token} from ${from?.label} to ${to?.label}, then pay ${intent.recipient}` : "I need an explicit amount, source, destination, and recipient before preparing a route.", amount: intent.amount, token: intent.token, fromChain: intent.fromChain, toChain: intent.toChain, recipient: intent.recipient, recipientLabel: intent.recipientLabel, estimatedFeeUsd: complete ? 0.18 : undefined, estimatedTime: complete ? "~18s" : undefined, route: complete ? [from?.short || "Source", "→", to?.short || "Destination", intent.recipientLabel || "Pay"] : undefined, plan: complete ? [{ id: "analyze", label: "Validate route" }, { id: "bridge", label: "Transfer USDC" }, { id: "settle", label: "Verify destination settlement" }, { id: "pay", label: `Pay ${intent.recipientLabel || "recipient"}` }] : [{ id: "details", label: "Specify all route details" }], canExecute: complete, requiresWallet: true };
    }
    default:
      return undefined;
  }
...
export async function orchestrateUserMessage(content: string, options?: { execute?: boolean }): Promise<OrchestratorResult> {
  ...
  const message: ChatMessage = { id: uid("msg"), role: "assistant", content: narrative(intent, preview), createdAt: new Date().toISOString(), intent: intent.type, actionPreview: options?.execute ? undefined : preview, transactionId: transaction?.id, codeBlocks };
  if (transaction?.status === "success") message.content += `\n\n✅ **Completed** — ${transaction.type} of ${transaction.amount} ${transaction.token}${transaction.recipientLabel ? ` to ${transaction.recipientLabel}` : ""}.`;
