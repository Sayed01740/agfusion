...
const ARC_SWAP_TOKENS = ["USDC", "EURC", "cirBTC"] as const;
type ArcSwapToken = (typeof ARC_SWAP_TOKENS)[number];
...
function normalizeQuote(raw: unknown): Quote {
  const amountOut = pickString(raw, ["amountOut", "amountOutFormatted", "estimatedAmountOut", "outputAmount"]);
  const fee = pickString(raw, ["fee", "feeAmount", "networkFee"]);
  const feeUsd = pickString(raw, ["feeUsd", "feeUSD", "estimatedFeeUsd"]);
  const priceImpact = pickString(raw, ["priceImpact", "priceImpactPct"]);
  const slippage = pickString(raw, ["slippage", "slippageBps"]);
  const route = pickString(raw, ["route", "provider", "source"]);
  return { amountOut, fee, feeUsd, priceImpact, slippage, route, raw };
}
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-white/[0.07] bg-black/10 p-3">
        <div className="flex items-end gap-2">
          <label className="min-w-0 flex-1">
            ...
            <select
              value={tokenIn}
              disabled={quoteBusy || swapBusy}
              onChange={(e) => { setTokenIn(e.target.value as ArcSwapToken); setQuote(null); setError(null); }}
              className="h-11 w-full rounded-xl border border-white/10 bg-slate-950/70 px-3 text-sm font-semibold text-slate-100"
            >
              {ARC_SWAP_TOKENS.map((token) => <option key={token}>{token}</option>)}
            </select>
          </label>
          <Button type="button" variant="outline" size="sm" className="mb-0.5 h-10 w-10 shrink-0 px-0 rounded-xl" disabled={quoteBusy || swapBusy} onClick={flip} aria-label="Reverse swap">
          ...
          <label className="min-w-0 flex-1">
            ...
            <select
              value={tokenOut}
              disabled={quoteBusy || swapBusy}
              onChange={(e) => { setTokenOut(e.target.value as ArcSwapToken); setQuote(null); setError(null); }}
              className="h-11 w-full rounded-xl border border-white/10 bg-slate-950/70 px-3 text-sm font-semibold text-slate-100"
            >
              {ARC_SWAP_TOKENS.map((token) => <option key={token}>{token}</option>)}
            </select>
      ...
      {quote && (
        <div className="rounded-xl border border-cyan-400/15 bg-cyan-400/[0.04] p-3.5 space-y-2.5">
          <div className="flex items-center justify-between gap-3">
            <span className="text-[11px] text-slate-400">Estimated receive</span>
            <span className="text-sm font-semibold text-slate-50">{prettyAmount(quote.amountOut)} {tokenOut}</span>
          </div>
          <div className="grid grid-cols-2 gap-2 text-[11px]">
            <div className="rounded-lg border border-white/[0.05] bg-black/10 p-2.5">
              <p className="text-slate-500">Fee</p>
              <p className="mt-0.5 text-slate-200">{quote.feeUsd || quote.fee || "Included in quote"}</p>
            </div>
            <div className="rounded-lg border border-white/[0.05] bg-black/10 p-2.5">
              <p className="text-slate-500">Price impact</p>
              <p className="mt-0.5 text-slate-200">{quote.priceImpact || "Quoted by Circle"}</p>
            </div>
          </div>
          <div className="flex items-center gap-2 text-[10px] text-slate-500">
            <ShieldCheck className="h-3.5 w-3.5 text-emerald-400" />
            Live quote from Circle App Kit · Arc Testnet
      ...
      {error && (
        <div className="rounded-xl border border-red-400/15 bg-red-400/[0.04] px-3 py-2.5 text-[11px] leading-relaxed text-red-200 whitespace-pre-wrap">
          {error}
        </div>
      )}

      {!quote && (
        <Button className="w-full" disabled={!canQuote || quoteBusy || swapBusy} onClick={() => void getQuote()}>
          {quoteBusy ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Getting live quote…</> : "Get live quote"}
        </Button>
      )}

      {quote && (
        <div className="space-y-2">
          <Button className="w-full" disabled={swapBusy || !walletAddress} onClick={() => void swap()}>
            {swapBusy ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Waiting for wallet…</> : "Continue · Confirm swap"}
          </Button>
          <Button type="button" variant="outline" className="w-full" disabled={swapBusy} onClick={() => void getQuote()}>
            <RefreshCw className="mr-2 h-3.5 w-3.5" /> Refresh quote
          </Button>
        </div>
      )}

      <div className="flex items-start gap-2 rounded-xl border border-white/[0.05] bg-white/[0.015] px-3 py-2.5 text-[10px] leading-relaxed text-slate-500">
        <CheckCircle2 className="mt-0.5 h-3.5 w-3.5 shrink-0 text-cyan-400/80" />
        Swap is same-chain on Arc Testnet. The app will not mark the operation successful until the transaction receipt is verified on-chain.
      </div>
    </div>
  );
}
