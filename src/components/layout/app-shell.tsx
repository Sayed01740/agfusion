...
export function AppShell({ children }: { children: React.ReactNode }) {
  return (
    <WalletProvider>
      <div className="ag-premium-shell min-h-screen mesh-bg overflow-x-clip relative">
        <div className="pointer-events-none fixed inset-0 grid-bg opacity-60 z-0" aria-hidden="true" />
        <div className="relative z-[1] min-w-0">
          <Navbar />
          <main className="pb-24 md:pb-10 min-w-0">{children}</main>
          <footer className="hidden md:block mt-10 border-t border-slate-200">
            <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-3 px-4 py-7 text-[11px] text-slate-500 sm:flex-row sm:px-6">
              <span className="inline-flex items-center gap-2.5">
                <Image src="/icon-32.png" alt="" width={16} height={16} className="rounded-sm opacity-90 ring-1 ring-blue-200" />
                <span>
                  <span className="font-display font-semibold text-slate-700">AGFusion</span>
