...
function toTxSteps(steps: SdkStep[] | undefined): TxStep[] { return (steps || []).map((step) => ({ name: step.name || "Bridge step", state: step.state === "success" ? "success" : step.state === "error" ? "error" : step.state === "noop" ? "success" : "pending", txHash: step.txHash, message: step.errorMessage })); }
function assertRoute(fromChain: ChainId, toChain: ChainId): void { if (!CIRCLE_BRIDGE_CHAINS.includes(fromChain) || !CIRCLE_BRIDGE_CHAINS.includes(toChain)) throw new Error("Circle Email Wallet bridge supports only configured CCTP routes."); if (fromChain === toChain) throw new Error("Source and destination must be different chains."); }
...
function isCompleted(result: SdkBridgeResult): boolean { return result.state === "success" && !!findStep(result.steps, ["mint", "receive", "destination"]); }
...
export async function runCircleEmailWalletForwardingBridge(params: { amount: string; fromChain: ChainId; toChain: ChainId; recipient?: string; txId?: string; }): Promise<TransactionRecord> {
  const debugId = startBridgeDebugSession(params.txId, { action: "circle-cctp-bridge", amount: params.amount, fromChain: params.fromChain, toChain: params.toChain, recipient: params.recipient });
  ...
  try {
    ...
    burnHandler = (payload: unknown) => { log("sdk.event.bridge.burn", payload); void (async () => { try { log("destination.switch.start", { from: sourceChainId, to: destinationChainId }); await switchToChainId(provider, params.toChain); const after = await getChainId(provider); log("destination.switch.complete", { expected: destinationChainId, actual: after }); if (after !== destinationChainId) throw new Error(`Wallet did not switch to destination chain ${params.toChain}.`); } catch (error) { log("destination.switch.error", { error }, error instanceof Error ? error.message : String(error)); } })(); };
    wildcardHandler = (payload: unknown) => log("sdk.event.*", payload);
    kit.on("bridge.burn", burnHandler); kit.on("*", wildcardHandler); log("sdk.listeners.installed", { events: ["bridge.approve", "bridge.burn", "bridge.attestation", "bridge.mint", "*"] });
    ...
    const record = buildRecord(result, state, { amount: params.amount, fromChain: params.fromChain, toChain: params.toChain, recipient }); log("bridge.record.created", { status: record.status, txHash: record.txHash, steps: record.steps, message: record.message });
  ...
  finally { if (burnHandler && kit) { try { kit.off("bridge.burn", burnHandler); } catch {} } if (wildcardHandler && kit) { try { kit.off("*", wildcardHandler); } catch {} } stopGlobal(); log("diagnostics.end"); }
}
