...
const KEY = "agfusion_bridge_debug_v5";
...
export function startBridgeDebugSession(txId?: string, context: Record<string, unknown> = {}): string {
  if (typeof window === "undefined") return txId || "";
  activeSessionId = txId || newId();
  clearBridgeDebugEvents();
  recordBridgeDebug("bridge.session.started", { context, url: location.href, userAgent: navigator.userAgent, timezone: Intl.DateTimeFormat().resolvedOptions().timeZone }, activeSessionId, "Bridge diagnostic session started");
...
export function installBridgeGlobalDiagnostics(txId?: string): () => void {
  if (typeof window === "undefined") return () => {};
  const id = txId || activeSessionId || newId();
  activeSessionId = id;
  const onError = (e: ErrorEvent) => recordBridgeDebug("window.error", { message: e.message, filename: e.filename, lineno: e.lineno, colno: e.colno }, id, e.message, { error: e.error });
  ...
  recordBridgeDebug("diagnostics.installed", { version: 5 }, id, "Bridge diagnostics installed");
...
export function clearBridgeDebugEvents(): void { if (typeof window !== "undefined") { try { window.localStorage.removeItem(KEY); window.sessionStorage.removeItem(KEY); } catch {} } }
export function downloadBridgeDebugLog(): void {
  if (typeof window === "undefined") return;
  const events = getBridgeDebugEvents();
  const payload: BridgeDebugSession = { exportedAt: new Date().toISOString(), app: "AGFusion", purpose: "Circle CCTP bridge diagnostic log", diagnosticVersion: 5, sessionId: activeSessionId || events[0]?.txId || newId(), eventCount: events.length, context: { url: location.href, userAgent: navigator.userAgent }, events };
  const blob = new Blob([JSON.stringify(payload, null, 2)], { type: "application/json" });
  const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = `agfusion-bridge-debug-v5-${Date.now()}.json`; document.body.appendChild(a); a.click(); a.remove(); setTimeout(() => URL.revokeObjectURL(url), 1000);
}
