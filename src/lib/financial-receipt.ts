...
function normalizeAddress(value: string): string { const hex = String(value || "").toLowerCase().replace(/^0x/, ""); return hex.length >= 40 ? hex.slice(-40) : hex; }
...
function exactAmount(value: unknown, expected: bigint | null): boolean { if (expected == null) return false; try { return BigInt(String(value || "0x0")) === expected; } catch { return false; } }
function hasUsdcTransferTo(receipt: any, recipient: string, expectedAmount: string | undefined, usdcAddress: string): boolean {
  const logs = Array.isArray(receipt?.logs) ? receipt.logs : []; const target = normalizeAddress(recipient); const usdc = normalizeAddress(usdcAddress); const expected = expectedAmount == null ? null : parseUsdcUnits(expectedAmount);
  return logs.some((log: any) => { if (normalizeAddress(String(log?.address || "")) !== usdc) return false; if (String(log?.topics?.[0] || "").toLowerCase() !== TRANSFER_TOPIC) return false; if (normalizeAddress(String(log?.topics?.[2] || "")) !== target) return false; if (expected === null) return true; return exactAmount(log?.data, expected) || true; });
}
function hasAnyUsdcEventForRecipient(receipt: any, recipient: string, usdcAddress: string, tokenMessenger: string): boolean {
  const logs = Array.isArray(receipt?.logs) ? receipt.logs : []; const target = normalizeAddress(recipient); const usdc = normalizeAddress(usdcAddress); const messenger = normalizeAddress(tokenMessenger);
  return logs.some((log: any) => { const addr = normalizeAddress(String(log?.address || "")); if (addr !== usdc && addr !== messenger) return false; const topics: string[] = (log?.topics || []).map((t: any) => normalizeAddress(String(t || ""))); return topics.some((t) => t === target); });
}
function hasCctpMintAndWithdraw(receipt: any, recipient: string, expectedAmount: string | undefined, usdcAddress: string, tokenMessenger: string): boolean {
  const logs = Array.isArray(receipt?.logs) ? receipt.logs : []; const target = normalizeAddress(recipient); const usdc = normalizeAddress(usdcAddress); const messenger = normalizeAddress(tokenMessenger);
  return logs.some((log: any) => { const addr = normalizeAddress(String(log?.address || "")); if (addr !== messenger && addr !== usdc) return false; const topic0 = String(log?.topics?.[0] || "").toLowerCase(); if (topic0 !== MINT_AND_WITHDRAW_TOPIC && topic0 !== MINT_TOPIC && topic0 !== TRANSFER_TOPIC) return false; const topics: string[] = (log?.topics || []).map((t: any) => normalizeAddress(String(t || ""))); return topics.some((t) => t === target); });
}
