...
export async function persistTransaction(
  ...
  try {
    ...
    const clientId = String(tx.id || "").trim();
    if (!clientId) {
      console.warn("[tx-store] refusing to persist transaction without a stable clientId");
      return null;
    }
    ...
    // clientId is the operation id generated by the transaction flow. Upsert
    // makes retries safe: the same logical transaction updates its existing
    // record instead of creating a duplicate row.
