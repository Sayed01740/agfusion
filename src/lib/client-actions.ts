...
export async function executeBridgeRecovery(params: { amount: string; fromChain: ChainId; toChain: ChainId; token?: string; recipient?: string; failedTx?: TransactionRecord | null; txId?: string }): Promise<TransactionRecord> {
  await assertAgentPolicy(params.amount, "bridge", params.recipient);
  if (getActiveWalletMeta()?.uuid === "circle-pw") return runCircleEmailWalletForwardingBridge({ amount: params.amount, fromChain: params.fromChain, toChain: params.toChain, recipient: params.recipient, txId: params.txId });
  const result = await runBridgeWithRecovery({ amount: params.amount, fromChain: params.fromChain, toChain: params.toChain, token: params.token, recipient: params.recipient, failedTx: params.failedTx, txId: params.txId });
  return finalizeVerifiedTransaction(result, params.toChain);
}
...
export function commandFromPreview(preview: ActionPreview): string {
  ...
  switch (preview.type) {
    ...
    case "swap": return `Swap ${amount} ${token} to ${preview.tokenOut || "EURC"}`;
    case "send": return preview.recipient ? `Send $${amount} ${token} to ${preview.recipient}` : `Send $${amount} ${token} — paste 0x address`;
    default: return `Show my balances`;
  }
}
