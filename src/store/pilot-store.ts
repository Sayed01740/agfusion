...
function normalizeTransactionForActivity(tx: TransactionRecord): TransactionRecord {
  const chain =
    tx.type === "bridge"
      ? tx.toChain === "Base_Sepolia"
        ? "base"
        : "arc"
      : tx.fromChain === "Base_Sepolia"
        ? "base"
        : "arc";
  ...
  return {
    ...
    explorerUrl:
      tx.explorerUrl || (tx.txHash ? explorerTxUrl(tx.txHash, chain) : undefined),
  };
}
...
export const usePilotStore = create<PilotState>((set, get) => ({
  ...
  addTransaction: (tx) => {
    set((s) => {
      const record = normalizeTransactionForActivity(tx);
      const transactions = [
        record,
  ...
  setWalletType: (t) => {
    try { window.localStorage?.setItem("agfusion_wallet_type_v1", t); } catch { /* ignore */ }
    set({ walletType: t });
  },
  ...
  loadServerTransactions: async (wallet) => {
    try {
      ...
      set((s) => {
        const merged = mergeTransactions(s.transactions, list).map(normalizeTransactionForActivity);
  ...
  markPreviewExecuted: (messageId) => set((s) => ({ messages: s.messages.map((m) => m.id === messageId && m.actionPreview ? { ...m, actionPreview: { ...m.actionPreview, executed: true } } : m) })),
  hydrate: () => {
  ...
  executionMode: () => "live",
  clearTransactions: () => { clearStoredTransactions(get().walletAddress); set({ transactions: [] }); },
}));
